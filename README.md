# Ghost for GovHack 2016

![GovHack logo](http://www.govhack.org/wp-content/themes/parallelus-salutation/images/logo.png)

We're going with Ghost as our blogging platform. Ghost is a simplistic Markdown-based blog engine. 

This repo contains a clone of the entire Ghost instance (as at 0.7.4). Customised components include our choice of sqlite3 for persistence (database is located at `/content/data`) and also the custom theme at `/content/themes/casper-clone/`. We'll keep it this way, for maximum portability of the blog between instances.

To stand up the blog, recommended actions are:

+ Route the blog through an AWS ELB, and expose the 'native' Ghost port internally on the EC2 instance
+ Use [pm2](https://github.com/Unitech/pm2) to manage the long-running Ghost process
